"""
New South Wales Government Job Scraper

Scrapes job postings from https://iworkfor.nsw.gov.au/jobs
"""
...
from .models import NSWJob, NSWScrapingMetadata
...
# Set up logging
log_filename = LOGS_DIR / f"nsw_scraper_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
...
def search_jobs(page: Page, keyword: str) -> Tuple[int, List[dict]]:
    ...
    # Navigate to jobs page (slower loading site, increased timeout)
    page.goto(config.JOBS_URL, wait_until="load", timeout=60000)
    time.sleep(config.PAGE_DELAY)
    
    # Find and fill search input
    search_input = page.locator('input.form-control[placeholder*="Enter keywords"]')
    search_input.fill(keyword)
    time.sleep(1)
    
    # Click search button
    search_button = page.locator('button.btn.btn-primary.search-btn:has-text("Search")')
    search_button.click()
    
    # Wait for results to load (give extra time for slow site to process search)
    time.sleep(config.PAGE_DELAY + 4)
    ...
    while True:
        ...
        if parser.has_next_page(html_content):
            try:
                ...
                # Click the "Next" button (use .first since there are 2 pagination controls on page)
                next_button = page.locator('button[aria-label="Pagination - Go to Next"][title="Next"]:not(.disabled)').first
                next_button.click()
...
def scrape_job_details(
    ...
    try:
        ...
        # Navigate to job page (increased timeout for slow loading)
        page.goto(job_url, wait_until="load", timeout=60000)
        
        # Wait for Blazor to load the content - wait for the job summary table
        try:
            page.wait_for_selector('table.table.table-striped.job-summary', timeout=45000)
            logger.debug("   ✅ Job content loaded")
        except Exception as e:
            logger.warning(f"   ⚠️  Timeout waiting for job content, continuing anyway...")
        ...
        job = NSWJob(
            ...
            job_category=job_details.get('job_category'),
...
def main():
    ...
    logger.info("🦘 New South Wales Government Job Scraper")
    logger.info("=" * 80)
