"""
Upload Ontario Jobs to Supabase

This script reads JSON files from data/ONT/jobs_json/ and uploads them
to the Supabase database using the ont_jobs table schema.
"""
...
DATA_DIR = PROJECT_ROOT / "data" / "ONT" / "jobs_json"
...
def upload_job(client: Client, job_data: Dict[str, Any]) -> bool:
    ...
    try:
        ...
        response = client.table('ont_jobs').upsert(job_data).execute()
...
def upload_all_jobs(limit: Optional[int] = None, dry_run: bool = False):
    ...
    # Get all JSON files
    json_files = list(DATA_DIR.glob("ont_job_*.json"))
    total_files = len(json_files)
    
    if total_files == 0:
        print("No JSON files found in data/ONT/jobs_json/")
        return
    ...
    for i, filepath in enumerate(json_files, 1):
        ...
        if dry_run:
            ...
            print(f"✓ Valid: {job_id} - {title} ({city})")
            successful += 1
        else:
            # Upload to Supabase
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Upload Ontario job postings to Supabase",
...
Examples:
  ...
  python src/ONT/upload_ont_jobs.py --limit 10
...
Environment Variables:
  ...
  You can set these in a .env file in the project root or in your shell:
