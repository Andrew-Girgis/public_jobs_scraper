"""
Web scraper for Western Australia Government Jobs
...
# Setup logging
log_file = config.LOGS_DIR / f"wa_scraper_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
logging.basicConfig(
    level=logging.INFO,
            ...
            for keyword_idx, keyword in enumerate(keywords, 1):
                ...
                try:
                    ...
                    for job_idx, job_data in enumerate(jobs, 1):
                        ...
                        wa_job = WAJob(
                            job_id=job_id,
                            position_number=details.get('position_number') or job_data['position_number'],
                            job_title=job_title,
                            job_url=f"{config.BASE_URL}/{job_data['job_url']}",
                            agency=job_data['agency'],
                            branch=job_data.get('branch'),
                            location=details.get('location') or job_data['location'],
                            classification=job_data['classification'],
                            posting_date=job_data['posting_date'],
                            closing_date=details.get('closing_date') or job_data['closing_date'],
                            division_name=details.get('division_name'),
                        ...
                        with open(json_file, 'w', encoding='utf-8') as f:
                            json.dump(wa_job.to_dict(), f, indent=2, ensure_ascii=False)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Scrape WA Government Jobs")
